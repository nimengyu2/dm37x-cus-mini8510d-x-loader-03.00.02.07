...
void config_3430sdram_ddr(int mcp_type)
{
...
#define EXTERNAL_WARM_RST                   (0x1 << 6)
#define RSTCTRL_RST_DPLL3                   (1 << 2)
#define RSTCTRL_RST_GS                      (1 << 1)

	if (__raw_readl(PRM_RSTST) & EXTERNAL_WARM_RST) {
		__raw_writel(__raw_readl(PRM_RSTCTRL) | (RSTCTRL_RST_DPLL3 | RSTCTRL_RST_GS), PRM_RSTCTRL);
		// WarningLed(LED4);
	}
...
void WarningLed(int led_nr) {
	...
	if (led_nr == -1) {
		omap_set_gpio_direction(LED1, 0);
		omap_set_gpio_dataout(LED1, 1);
		omap_set_gpio_direction(LED2, 1);
		omap_set_gpio_dataout(LED2, 1);
		omap_set_gpio_direction(LED3, 0);
		omap_set_gpio_dataout(LED3, 1);
		omap_set_gpio_direction(LED4, 0);
		omap_set_gpio_dataout(LED4, 1);
		return;
	}

	omap_request_gpio(led_nr);
	omap_set_gpio_direction(led_nr, 0);
	omap_set_gpio_dataout(led_nr, 1);
	for (i = 0; i < 5; i++) {
		omap_set_gpio_dataout(led_nr, 0);
		delay(WARNING_DELAY);
		omap_set_gpio_dataout(led_nr, 1);
		delay(WARNING_DELAY);
	}
