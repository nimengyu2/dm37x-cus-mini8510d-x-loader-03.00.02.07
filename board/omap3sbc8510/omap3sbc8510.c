...
/******************************************
 * sbc8510_identify
 * Description: Detect if we are running on a SBC8510 revision Ax/Bx,
...
void config_3430sdram_ddr(void)
{
	...
	if (sbc8510_revision() == REVISION_XM) {
...
int misc_init_r(void)
{
	...
	rev = sbc8510_revision();
	switch (rev) {
	case REVISION_AXBX:
		printf("SBC8510 Rev Ax/Bx\n");
		break;
	case REVISION_CX:
		printf("SBC8510 Rev C1/C2/C3\n");
		break;
	case REVISION_C4:
		printf("SBC8510 Rev C4\n");
		break;
	case REVISION_XM:
		printf("SBC8510 xM Rev A\n");
		break;
	default:
		printf("SBC8510 unknown 0x%02x\n", rev);
	}
