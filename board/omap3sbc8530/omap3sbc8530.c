...
/******************************************
 * devkit8500_identify
 * Description: Detect if we are running on a SBC8530 revision Ax/Bx,
...
void config_3430sdram_ddr(void)
{
	...
	if (devkit8500_revision() == REVISION_XM) {
...
int misc_init_r(void)
{
	...
	rev = devkit8500_revision();
	switch (rev) {
	case REVISION_AXBX:
		printf("SBC8530 Rev Ax/Bx\n");
		break;
	case REVISION_CX:
		printf("SBC8530 Rev C1/C2/C3\n");
		break;
	case REVISION_C4:
		printf("SBC8530 Rev C4\n");
		break;
	case REVISION_XM:
		printf("SBC8530 xM Rev A\n");
		break;
	default:
		printf("SBC8530 unknown 0x%02x\n", rev);
	}
